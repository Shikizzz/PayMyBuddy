...
/*    @GetMapping("/contact")
    public String getContact(@RequestParam Optional<String> error, Model model, HttpSession session) {
    ...
    public String getContact(@RequestParam Optional<String> error, @RequestParam("page") Optional<Integer> page,
        ...
        ArrayList<Connection> connections = new ArrayList<>(connectionsService.userToConnectionList(((User)session.getAttribute("user")).getConnections()));
        Page<Connection> connectionsPage = connectionsService.findPaginated(PageRequest.of(currentPage - 1, pageSize), connections);
    ...
    @Transactional
    @PostMapping("/contact")
    public String postContact(@ModelAttribute(name="email")String email, BindingResult result, Model model, HttpSession session, RedirectAttributes attributes){
        if (result.hasErrors()) {
            return "error";
        }
